...
/* image_height */
    if(isset($url[0])){
    	//$img_info = wp_remote_get($url[0]);
        list($width, $height) = getimagesize($url[0]);
        //print $width;
        //print_r($img_info[1]);
    	$data_image_height = " data-background-height='{$height}' data-background-width='{$width}'";
    }
...
/* Link Color */
    ...
    if($row_link_color || $row_link_color_hover || $row_head_color || $css){
