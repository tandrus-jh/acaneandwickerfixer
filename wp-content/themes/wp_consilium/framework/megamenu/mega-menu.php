...
function custom_nav_update($menu_id, $menu_item_db_id, $args ) {
    $fields = array('mega','bg_image','bg_image_attachment','bg_image_size','bg_image_position','bg_image_repeat','bg_color','menu_icon');
...
function add_js_mega_menu(){
    ...
    wp_enqueue_style( 'cs_font_awesome', '//netdna.bootstrapcdn.com/font-awesome/4.1.0/css/font-awesome.min.css' );
    wp_enqueue_media();
    add_thickbox();
}
    ...
    <li data-menuanchor="" id="menu-item-<?php echo $item_id; ?>" class="<?php echo implode(' ', $classes ); ?>">
        ...
        <div class="menu-item-settings" id="menu-item-settings-<?php echo $item_id; ?>">
            ...
            <p class="field-mega description description-wide">
                <label for="edit-menu-item-mega-<?php echo $item_id; ?>">
                    <?php _e( 'Mega menu' ,THEMENAME); ?><br />
                    <input type="checkbox" id="edit-menu-item-mega-<?php echo $item_id; ?>" class="widefat code edit-menu-item-mega" name="menu-item-mega[<?php echo $item_id; ?>]" value="1" <?php checked( $item->mega, '1' ); ?>/>
                </label>
            </p>
            <?php
        $title = 'Menu Icon';
        ...
        <p class="description description-wide obtheme_checkbox obtheme_mega_menu obtheme_mega_menu_d1">
            ...
            <label for="edit-<?php echo $key . '-' . $item_id; ?>">
                <?php _e( $title, 'cshero' ); ?><br />
                <input type="text" value="<?php echo $value; ?>" id="edit-<?php echo $key . '-' . $item_id; ?>" class=" <?php echo $key; ?>" name="<?php echo $key . "[" . $item_id . "]"; ?>" />
                <input alt="#TB_inline?height=400&width=500&inlineId=<?php echo $key . '-' . $item_id . '-popup'; ?>" title="<?php _e( 'Click to browse icon', 'cshero' ) ?>" class="thickbox button-secondary submit-add-to-menu" type="button" value="<?php _e( 'Browse Icon', 'cshero' ) ?>" />
            ...
            <p class="description description-wide obtheme_checkbox obtheme_mega_menu obtheme_mega_menu_d2">
                <label for="edit-<?php echo $key . '-' . $item_id; ?>">
                    <span class='obtheme_long_desc'><?php _e( $title, 'cshero' ); ?></span><br />
                    <input type="text" value="<?php echo $value; ?>" id="edit-<?php echo $key . '-' . $item_id; ?>" class=" <?php echo $key; ?>" name="<?php echo $key . "[" . $item_id . "]"; ?>" />
                    <button id="browse-edit-<?php echo $key . '-' . $item_id; ?>" class="set_custom_images button button-secondary submit-add-to-menu"><?php _e( 'Browse Image', 'cshero' ); ?></button>
                </label>
            </p>
...
class HeroMenuWalker extends Walker_Nav_Menu {
    ...
    function start_lvl( &$output, $depth = 0, $args = array() )  {
        $bg_image        = $args->element->bg_image;
        $bg_color        = $args->element->bg_color;
        $style = 'style="';
        $class = $bg_image?'sub-menu mega-bg-image':'sub-menu';
        if ( $bg_image ) {
    ...
    public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        if($item->menu_item_parent==0&&$item->mega==1){
            $classes[]='mega-menu-item';
        }else if($item->menu_item_parent==0&&$item->mega==0){
...
if(!function_exists('hero_get_mega_menu_option_element')){
    function hero_get_mega_menu_option_element(){
        $items = hero_get_main_menu_parent_items();
        ...
        if(!empty($items)){
            ...
            return array(
                ...
                'desc'=>'In order to enable Mega Menu items you have to create a custom menu
