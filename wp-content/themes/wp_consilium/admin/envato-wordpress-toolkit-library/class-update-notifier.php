...
class UpdateNotifier {
	...
	const NONCE = 'update_nonce';
	const ENVATO_LIB_PATH = 'admin/envato-wordpress-toolkit-library/class-envato-wordpress-theme-upgrader.php';
	const UPDATE_ATTRIBUTE = 'theme_update';
	/**
	 * Main constructor.
	 *
	...
	public function add_admin_bar_noification() {
		if ( function_exists( 'simplexml_load_string' ) ) { // Stop if simplexml_load_string funtion isn't available
			...
			if ( $this->is_new_version_available() ) { // Compare current theme version with the remote XML version
				$wp_admin_bar->add_menu( array( 'id' => 'update_notifier', 'title' => '<span>' . $this->theme_name . ' <span id="ab-updates">New Updates</span></span>', 'href' => get_admin_url() . 'index.php?page='.$this->update_page_name ) );
			}
		}
	}
	/**
	...
	public function get_update_xml() {
		if ( !isset( $this->xml ) ) {
			$db_cache_field = 'notifier-cache-'.$this->short_name;
			$db_cache_field_last_updated = 'notifier-cache-last-updated-'.$this->short_name;
			$last = get_option( $db_cache_field_last_updated );
			$now = time();
			// check the cache
			if ( !$last || ( ( $now - $last ) > $this->cache_interval ) ) {
	...
	public function init_update_functionality() {
		if ( isset( $_GET[self::UPDATE_ATTRIBUTE] ) &&  $_GET[self::UPDATE_ATTRIBUTE]=='true' ) {
			...
			$tf_username = get_theme_mod( 'username_evt' );
			$tf_api_key = get_theme_mod( 'api_key_evt' );
			...
			$check_for_theme_update = $upgrader->check_for_theme_update($this->theme_name, $allow_cache);
	...
	public function add_update_notices() {
		...
		if ( isset( $this->theme_updated ) ) {
			if ( $this->theme_updated ) {
				...
				$message = isset($this->update_message)? $this->update_message : 'An error occurred, the theme has not
	...
	protected function print_instructions() {
		...
		$envato_details = ( get_theme_mod('username_evt') && get_theme_mod('api_key_evt') )?"true":"false";
		$template_directory_uri = get_template_directory_uri();
		wp_register_script( 'update-script', get_template_directory_uri() . '/admin/envato-wordpress-toolkit-library/js/theme-update-script.js' );
		wp_localize_script( 'update-script', 'theme_update', array('update_url'=>$update_url,'envato_details'=>$envato_details));
		wp_enqueue_script( 'update-script' );
		echo '
		<div id="instructions">
			<div class="two-columns">
				...
				1. Go to <strong>"Theme Options" &raquo; "Theme Update"</strong> section and insert your Envato Marketplace username and API Key in the relevant fields. <br/>
